...
from .music import initialize, start, pause, unpause, stop, skip_to, go_back, set_volume, gpos, continue_
...
class MusicPlayer(Tk):
    def __init__(self, *args, **kwargs):
        ...
        play = Button(self, text="Play", font=FONT, command=lambda: start(files[playing]))
        play.pack()
        song_info = Label(self, font=FONT)
        song_info.pack()
        volume = Scale(self, label="Volume:", font=FONT, from_=0, to=100, variable=vol, orient="horizontal", command=lambda _volume: set_volume(int(_volume) / 100))
        ...
        def add(folder:bool=None):
            ...
            if folder is None:
                ...
                if info.extension[0] == "mp3" and info.type[0] == "audio" and get_prompt.name not in files:
                ...
                if get_prompt:
                    ...
                    if fid:
                        for file in fid:
                            ...
                            if info.extension[0] == "mp3" and info.type[0] == "audio" and file_path not in files:
                ...
                if get_prompt:
                    for file in get_prompt:
                        ...
                        if info.extension[0] == "mp3" and info.type[0] == "audio" and file.name not in files:
